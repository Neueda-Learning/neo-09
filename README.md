...
that role can only touch this repo's own `neobank-<env>-neo-09` stack. You never hold AWS
credentials yourself — everything that reaches AWS goes through a workflow in this repo.

The front-end image is built with `APP_BASE_PATH=/neo-09`, because in the deployed stack
...
|---|---|---|---|
| **dev** | [`/neo-09/`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-09/) | [`/neo-09/health`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-09/health) · [`/neo-09/info`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-09/info) | [orchestrator](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/) |
| **prod** | [`/neo-09/`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-09/) | [`/neo-09/health`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-09/health) · [`/neo-09/info`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-09/info) | [orchestrator](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/) |
...
A `reset` destroys **only this service's schema** (`neo_09`). Every other service and the
...
DB_URL=jdbc:mysql://localhost:3307/neo_09 ./mvnw spring-boot:run
```
...
```json
{ "serviceId": "neo09", "status": "ACCEPTED",
...
| Env | Default | What |
|---|---|---|
| `SERVICE_ID` | `neo09` | the id sent on callbacks — note **no `-a`**, unlike the repo name |
| `SERVICE_NAME` | `Customer Support` | display name |
...
- `backend/` and `frontend/` **stay at the repo root** — the system compose builds
  `./neo-09/backend` and `./neo-09/frontend` by path.
